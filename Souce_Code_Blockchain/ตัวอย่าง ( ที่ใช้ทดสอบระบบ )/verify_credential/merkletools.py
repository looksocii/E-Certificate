...
class MerkleTools(object):
    ...
    def _calculate_next_level(self):
        ...
        for l, r in zip(self.levels[0][0:N:2], self.levels[0][1:N:2]):
            ...
            if length == 1:
                leftNode = Node(None, self._to_hex(l), None, None)
                rightNode = Node(None, self._to_hex(r), None, None)
                self.nodes.append(leftNode)
                self.nodes.append(rightNode)
            node = Node(None, self._to_hex(value), self._to_hex(l), self._to_hex(r))
        ...
        if solo_leave is not None:
            ...
            if length == 1:
                leftNode = Node(None, self._to_hex(solo_leave), None, None)
                rightNode = Node(None, self._to_hex(solo_leave), None, None)
                self.nodes.append(leftNode)
                self.nodes.append(rightNode)
            node = Node(None, self._to_hex(value), self._to_hex(solo_leave), self._to_hex(solo_leave))
            self.nodes.append(node)
